...
| **ACTIVE PLAN — PR 1 of 3 BUILT 2026-08-28, pending owner local testing + commit** — calendar-month billing anchor (ADR 007: renewals on the 1st — current code was anniversary-billed, a discovered divergence from the intended CKQ model; proration made unconditional, `Setting.prorationEnabled` deprecated, `firstOfNextMonth()` added, `realign-billing-anchors` migration script added). PR 2 (one-active-subscription-per-student guard, ADR 005) and PR 3 (sibling-discount family rule, ADR 006) not yet started. | `docs/plans/billing-anchor-and-sibling-discount-plan.md` |
