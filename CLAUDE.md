...
| **PR 1 MERGED (PR #79); PR 2 BUILT, pending owner local testing + review; PR 3 not started** — airtight payments. Triggered by a real incident (a parent charged $403.82 instead of $420 via "Enroll for next month"). PR 1 (`billing/proration.service.js`'s `resolveFirstChargePeriod()` — proration now applies only to a CURRENT-calendar-month start date; any FUTURE month bills the full fee flat for that whole month, ADR 007 addendum) merged to develop 2026-08-30. PR 2 (admin Charge dialog now offers card-vs-manual × full-vs-prorated options; ledger `periodMonth` field re-keys Guard B to a true one-payment-per-subscription-per-calendar-month DB invariant across every charge pathway, incl. a dry-run-first migration script; method-aware receipt/invoice line) built 2026-08-30 on `feature/charge-modes-and-month-guard`, not yet committed. PR 3 (parent `/parent/billing` history page reading the `Registration` ledger — currently the only SOT for payment history; "Last Charge" → "Last Payment" fix on both parent and admin subscriptions pages) not started. | `docs/plans/payment-airtight-plan.md` |
