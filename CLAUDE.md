...
## Platform Scope (MVP)
Users/auth · group classes/schedules/sessions · pricing · attendance · trial + regular registration · in-house recurring subscription billing with a 10% sibling discount (family rule — highest-paying child excluded, everyone else discounted, applied immediately at registration; not limited to 2 children; `docs/decisions/006-sibling-discount-family-rule.md`).
...
| **PR 1 BUILT, pending owner local testing + review (PR 2 optional, owner's call)** — session start-time cutoff: a group-class session that has already started (not merely whose calendar day has passed) no longer appears in the trial/register pickers, is rejected as a trial or registration start date, gets no new roster Visit, and reads as a completed trial. Root cause: `GroupClassSession.date` was a day-only sentinel and nothing combined it with the schedule's `startTime`. Fix: `GroupClassSession` now also stores `startsAt`/`endsAt` real UTC instants (CKQ's model; the same shape private classes already used), written by the one generator; every "started/upcoming" query is `startsAt` vs a real `now`. Roster helpers dropped their `today` param; `PUT /group-class-schedules/:id` re-resolves future instants on a time change and rejects `dayOfWeek` changes; new dry-run-first `scripts/backfill-session-instants.js` (**run on staging, then production, BEFORE deploying** — `startsAt`/`endsAt` are `required`). Built 2026-09-23 on `feature/session-start-time-cutoff`; full backend (71 suites) + frontend (418) green, `tsc --noEmit` clean. Not yet committed. PR 2 (group attendance "not started" gate) is spec'd but unbuilt. | `docs/plans/session-start-time-cutoff-plan.md` |
| **BOTH PRs BUILT, all suites green, pending owner local testing + review** — holiday blocking, adapted from CKQ's Holiday feature and deliberately simplified (no billing interaction at all — a holiday never changes a monthly fee, proration, or renewal, unlike CKQ's own version). New `Holiday` model/service/routes (admin/superadmin-only CRUD, incl. list), calendar-day-sentinel dates, ≤31-day duration cap + unique-name + inclusive-overlap validation. PR 1 (backend, `feature/holiday-blocking-backend`): `listUpcomingByClass` server-filters holiday dates out of both the trial and registration start-date pickers (one shared endpoint, one fix); `markAttendance`/the walk-in `addStudentToSession` both 400 on a holiday-date session; `registration.service.js`'s `resolveStartDate` and `trialClass.service.js`'s `create` both 400 as defense in depth; `listBySchedule`/`getById` annotate `isHoliday`/`holidayName` (additive) for display. Built + full backend suite verified green under `TZ=UTC` (775/775 tests, zero regressions, including the real-Stripe-network registration suite). PR 2 (frontend, `feature/holiday-blocking-frontend`): new Pattern A `/admin/holidays` page (through the shared `Modal`), a new **Programs** nav entry, admin/coach sessions-list pages render a holiday row muted with a `Holiday — <name>` chip and no attendance link, the shared attendance page renders a blocked alert with no roster/Save for an `isHoliday` session. `tsc --noEmit` clean, `next build` succeeds, full frontend suite green (400/400), new `frontend/e2e/holiday-blocking.spec.ts` (5 tests: admin CRUD round-trip, both wizards' pickers never offer a blocked date, coach attendance blocked state, admin sessions list holiday row) plus `admin-shell.spec.ts`'s nav assertion updated — full E2E suite green (26/26 active). Not yet committed. | `docs/plans/holiday-blocking-plan.md` |
| **PR 1 + PR 2 MERGED (PR #79, #80); PR 3 BUILT, pending owner local testing + review — ALL 3 PRs now built** — airtight payments. Triggered by a real incident (a parent charged $403.82 instead of $420 via "Enroll for next month"). PR 1 (`billing/proration.service.js`'s `resolveFirstChargePeriod()` — proration now applies only to a CURRENT-calendar-month start date; any FUTURE month bills the full fee flat for that whole month, ADR 007 addendum) merged to develop 2026-08-30. PR 2 (admin Charge dialog now offers card-vs-manual × full-vs-prorated options; ledger `periodMonth` field re-keys Guard B to a true one-payment-per-subscription-per-calendar-month DB invariant across every charge pathway, incl. a dry-run-first migration script — not yet run anywhere; method-aware receipt/invoice line) merged to develop 2026-08-30. PR 3 (new `/parent/billing` payment-history page + `PaymentHistoryTable` component reading ONLY the `Registration` ledger via a new `GET /registrations/history`; portal nav split "Billing" into "My Registrations" + "Payment History"; "Last Charge" → "Last Payment" fix, ledger-sourced and fee-included, on both parent and admin subscriptions pages) built 2026-08-31 on `feature/parent-billing-history`, not yet committed. | `docs/plans/payment-airtight-plan.md` |
| **SHIPPED plan** — UTC date-storage standard (ADR `docs/decisions/009-utc-date-storage-standard.md`): fixed the root cause of a reported trial-picker Sunday-for-Monday display bug at every layer, not just the display. PR 1 (`frontend/lib/formatDate.ts`, every sitewide `toLocaleDateString`/`toLocaleString` call site migrated, incl. the register wizard's start-date window math) merged 2026-08-30 (PR #76). PR 2 (`backend/src/utils/dateShapes.js`, `GroupClassSession.date` generator + `listUpcomingByClass` + every roster-mutation call site rebuilt on calendar-day sentinels, `email/dates.js`'s `dateOnlyFull` + `monthLabel` fix — also fixed a live server-side bug beyond the original report, the trial-confirmation email stating the wrong day — plus a tested, dry-run-first migration script) merged 2026-08-30 (PR #77). PR 3 (`PrivateClassSession.startDate`/`endDate` rebuilt on `combineDayAndTimeInTZ`, DST-safe weekly stepping — fixes a live bug where the per-session Stripe charge gate opened hours before the lesson actually happened; confirmed against real staging data) — migration deliberately NOT built for this PR (owner decision 2026-08-30: no real private-class data exists anywhere to migrate). Full backend suite green throughout (685 tests at PR 3), full frontend suite + E2E green at PR 1. | `docs/plans/utc-date-standard-plan.md` |
| **BOTH PRs BUILT, pending owner local testing + review** — manual Charge button + PDF invoices. PR 1 (superadmin-only `/admin/subscriptions` Charge action; `renewal.service.js`'s `previewRenewal`/`chargeNow` reuse `renewOne`/`retryOne` verbatim, zero new charge logic) built 2026-08-30 on `feature/manual-charge-button`. PR 2 (PDF invoice — `pdfkit` + `backend/src/services/invoice.service.js` — on every completed charge, group class AND private lesson, attached to the confirmation/renewal/private-session receipt emails via a nested try/catch that never blocks the email or the charge, plus an on-demand `GET /registrations/:id/invoice` download) built 2026-08-30 on `feature/pdf-invoices`, stacked on PR 1 (PR 1 not yet merged when PR 2 was built). Owner still needs to fill the real EIN + address into `backend/src/config/academy.js` (ships with placeholders). Backend suites green for every file this work touched (including a pre-existing, independently `git stash`-reverified 14-test registration-routes failure block, unrelated — real-Stripe-minimum-charge errors from near-month-end proration on "today"-anchored fixtures, not caused by either PR); frontend unaffected by PR 2 (no UI), `tsc --noEmit` clean. See `docs/decisions/001-in-house-subscription-billing.md`'s 2026-08-30 addendum, `docs/features/admin.md`'s Subscriptions section, `docs/modules/email.md`'s "PDF invoice attachments" section, and `docs/features/private-class.md`'s charge pipeline. | `docs/plans/manual-charge-and-pdf-invoice-plan.md` |
| **SHIPPED plan** — per-level one-time registration fee: optional `Price.registrationFee` overriding the academy-wide `Setting.registrationFee` (level wins when set, including an explicit `0`; falls back only when unset), both `registration.service.js` call sites updated, admin Prices page field/column + Settings relabel. MERGED TO DEVELOP 2026-08-29 (PR #69); full suites green (564 backend / 308 frontend), `tsc --noEmit` clean. Rollout (setting the real $145/$100 values in the admin UI) still owner-pending. | `docs/plans/per-level-registration-fee-plan.md` |
| **BUILT, pending owner local testing + review** — shared `Modal` component (`app/components/ui/Modal/`) fixing a sitewide bug: every dialog's backdrop click silently closed it and discarded the form. 25 hand-rolled dialog instances across 13 files migrated to render through one component with no backdrop-click-to-close, an `Escape` listener, and focus capture/restore; dead duplicated dialog CSS deleted from `admin.module.css` and `AddChildModal.module.css` (the latter file removed entirely). Built 2026-08-29 on `feature/shared-modal-component`; full suites green (frontend 322/322, E2E 19/19 + 2 known pre-existing skips), `tsc --noEmit` clean. Not yet committed. | `docs/plans/shared-modal-component-plan.md` |
| **READY-TO-EXECUTE plan** — WordPress UI alignment (rebrand to the live site's navy/crimson + Big Shoulders Text/Inter, public home restructured to mirror friscofencingacademy.com's section order, "Family Scorecard" checkout quote panel with a small backend `savings` addition to `GET /registrations/preview`). 3 phases = 3 PRs; spec'd 2026-08-29 from the owner's WP export; keeps all existing flows, backend stays billing SOT. | `docs/plans/wordpress-ui-alignment-plan.md` |
| **SHIPPED plan** — all 3 PRs merged to `develop` 2026-08-28: PR 1 calendar-month billing anchor (ADR 007, PR #50); PR 2 one-active-subscription-per-student guard + create-pending-first registration (ADR 005/008 — found mid-build that tightening the guard exposed a real cross-schedule double-charge race, closed by reserving the `Subscription` before ever calling Stripe, PR #51); PR 3 sibling-discount family rule (ADR 006 — top-payer-excluded, immediate at registration incl. the higher-payer "bridge" case, PR #52). | `docs/plans/billing-anchor-and-sibling-discount-plan.md` |
