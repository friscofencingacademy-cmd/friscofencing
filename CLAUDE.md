...
| **BUILT, pending owner local testing + review** — per-level one-time registration fee: optional `Price.registrationFee` overriding the academy-wide `Setting.registrationFee` (level wins when set, including an explicit `0`; falls back only when unset), both `registration.service.js` call sites updated, admin Prices page field/column + Settings relabel. Built 2026-08-29 on `feature/per-level-registration-fee`; full suites green (564 backend / 308 frontend), `tsc --noEmit` clean. Not yet committed. | `docs/plans/per-level-registration-fee-plan.md` |
