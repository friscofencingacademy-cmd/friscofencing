...
| **Shipped plan** — Service registry + unified two-dimension payment ledger: `Service` model (stable `code` + declared `billingShape` — CKQ's registry with its name-string-as-key wart fixed), `Registration` restructured into a shared money-contract base + three billing-shape discriminators (`subscription_cycle`/`per_session`/`one_time_event`), `PrivateClassCharge` absorbed and retired, camps/meets covered schema-only for the future. PR A + PR B both built and verified (426/428 backend tests, the 2 failures are the pre-existing unrelated proration bug), on `feature/service-registry` + `feature/unified-ledger`, awaiting owner diff review before commit/ship. [ADR 004](./docs/decisions/004-service-registry-and-unified-ledger.md) records the design. | `docs/plans/service-registry-unified-ledger-plan.md` |
