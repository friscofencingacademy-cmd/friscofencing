...
| **NEXT-BATCH ORCHESTRATOR (execute this)** — three items in order for a Sonnet builder: (1) orphaned-reference fix (per its amended plan below), (2) registration-ledger PR 2 (renewal create-pending-first + stale-pending recovery), (3) PR 3 (retry/dunning + cancel-after-exhaustion) — with the concrete post-unified-ledger deltas (discriminator writes, `getServiceByCode` resolution, `seedServices()` in renewal tests) verified against current source 2026-08-28, so the older specs execute correctly against the restructured schema. | `docs/plans/next-batch-execution-plan.md` |
...
| **Approved plan (not started; AMENDED 2026-08-28)** — orphaned-reference crash fix, Item 1 of the next-batch orchestrator above: the live `/private-classes` staging 500 was DATA-FIXED 2026-08-28 (two orphaned free `PrivateClassSchedule` docs deleted by hand, owner-approved — endpoint verified 200), but the CODE fix is still needed so the next orphan degrades instead of crashing. Base plan: unguarded-populate audit (5 backend read paths, 6 frontend spots), coach-delete guard, D7 diagnostic script. §8 Amendment extends to student/parent orphans, the missing student-guard `PrivateClassEnrollment` check, and `reset-customer-data.js` cleanup. | `docs/plans/orphaned-coach-reference-fix-plan.md` |
