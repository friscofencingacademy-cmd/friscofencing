...
| **ACTIVE PLAN — PR 1 MERGED, PR 2 of 3 BUILT 2026-08-28, pending owner local testing + commit** — PR 1: calendar-month billing anchor (ADR 007), merged to `develop` via PR #50. PR 2: one-active-subscription-per-student guard (ADR 005) — found mid-build that tightening the guard exposed a real cross-schedule double-charge race under the old charge-first `create()`; closed by reordering `create()` to reserve the `Subscription` before ever calling Stripe and reusing renewal's own dunning for a failed first charge (ADR 008) — `chargeFinalization.service.js` extracted so both flows share one charge/ledger/period implementation. `POST /registrations` now returns a `paymentStatus: 'completed' \| 'pending'` field; the register wizard has a distinct "processing" screen for `pending`. PR 3 (sibling-discount family rule, ADR 006) not yet started. | `docs/plans/billing-anchor-and-sibling-discount-plan.md` |
