...
| **Approved plan (not started)** — Service registry + unified two-dimension payment ledger: new `Service` model (stable `code` + declared `billingShape` — CKQ's registry with its name-string-as-key wart fixed), `Registration` restructured into a shared money-contract base + three billing-shape discriminators (`subscription_cycle`/`per_session`/`one_time_event`), `PrivateClassCharge` absorbed and retired, camps/meets covered schema-only for the future. 2 PRs, spec'd for a Sonnet builder; MUST land before registration-ledger-plan's PR 2/3. New ADR 004 ships with it. | `docs/plans/service-registry-unified-ledger-plan.md` |
| **Approved plan (not started)** — orphaned coach reference crash fix: `/private-classes` 500s live on staging (confirmed) when a `PrivateClassSchedule`/`CoachContract`/`PrivateClassEnrollment` references a deleted coach; full audit found the same unguarded-populate pattern in 5 backend read paths and 6 frontend display spots, plus a missing coach-delete guard (root cause) and a second live crash site in enrollment creation. Spec written for a Sonnet builder. | `docs/plans/orphaned-coach-reference-fix-plan.md` |
