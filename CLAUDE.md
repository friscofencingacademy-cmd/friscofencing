...
| **NEXT-BATCH ORCHESTRATOR (Item 1 SHIPPED 2026-08-28, PR #44 — timezone plan below now runs before Item 2)** — three items in order for a Sonnet builder: (1) orphaned-reference fix ✅ SHIPPED + §8f live-verified on staging, (2) registration-ledger PR 2 (renewal create-pending-first + stale-pending recovery), (3) PR 3 (retry/dunning + cancel-after-exhaustion) — with the concrete post-unified-ledger deltas (discriminator writes, `getServiceByCode` resolution, `seedServices()` in renewal tests) verified against current source 2026-08-28, so the older specs execute correctly against the restructured schema. | `docs/plans/next-batch-execution-plan.md` |
| **SHIPPED plan (pending owner review), owner-sequenced 2026-08-28 before ledger PR 2 above** — timezone consistency: `todayAtMidnight()`/renewal cron's "today" gate was server-local (UTC in prod) instead of Central — same bug class CKQ's `dateUtils.js` fixed after 4 repeat incidents. Adds `moment-timezone` + `DEFAULT_TIMEZONE` (`config/timezone.js`); `todayAtMidnight`/`addOneDay`/`nextOccurrenceStrictlyAfter`/`nextOccurrenceOnOrAfter` now tz-aware; new `todayDateOnly()` fixes a second, distinct bug found mid-build in `registration.service.js`'s `anchorDate` fallback; `Location.timezone` gains IANA-format validation (still unwired to any call site — one location today). Two real, non-trivial gaps were caught and corrected DURING this build (not just planned in advance) — see the plan's D9/D10 for both. Built on `feature/timezone-consistency` 2026-08-28: 455/456 backend tests (the 1 failure is the pre-existing, independently-reverified-via-`git stash` unrelated $0-proration bug), 0 frontend files touched. Not yet merged to `develop`. | `docs/plans/timezone-consistency-plan.md` |
...
| **Active plan** — Registration → per-charge payment ledger (CKQ model): rebuilt `Registration` schema, two DB-level guards (active-Subscription uniqueness + ledger dedup index), create-pending-first renewal sequencing, stale-pending recovery, daily retry/dunning with cancel-after-3, migration script. PR 1 of 3 (schema+guards+initial-registration ledger writes+migration script) MERGED TO DEVELOP 2026-08-27 (PR #37); PR 2/3 (renewal sequencing, retry/dunning) not started. Companion finding: `docs/plans/registration-ledger-gap-analysis.md`. | `docs/plans/registration-ledger-plan.md` |
