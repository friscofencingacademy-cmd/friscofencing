...
| **PR 1 BUILT, pending owner local testing + review** — manual Charge button + PDF invoices, 2 PRs. PR 1 (superadmin-only `/admin/subscriptions` Charge action; `renewal.service.js`'s `previewRenewal`/`chargeNow` reuse `renewOne`/`retryOne` verbatim, zero new charge logic) built 2026-08-30 on `feature/manual-charge-button`; backend 586/606 + new suites green (20 pre-existing local-TZ-environment failures, reverified via `git stash` unrelated to this change), frontend 346/346, `tsc --noEmit` clean. PR 2 (PDF invoice on every completed charge, group AND private-class, attached to receipt emails + on-demand download) not yet started. See `docs/decisions/001-in-house-subscription-billing.md`'s 2026-08-30 addendum and `docs/features/admin.md`'s Subscriptions section. | `docs/plans/manual-charge-and-pdf-invoice-plan.md` |
