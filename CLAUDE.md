...
| **PLAN — ALL 3 PRs BUILT 2026-08-28, PR 3 pending owner local testing + commit** — PR 1: calendar-month billing anchor (ADR 007), merged via PR #50. PR 2: one-active-subscription-per-student guard (ADR 005) + create-pending-first registration (ADR 008, found mid-build closing a real cross-schedule double-charge race the guard alone exposed) — merged via PR #51. PR 3: sibling-discount family rule (ADR 006) — `calculateChargeAmount()` now takes an explicit `mode: 'registration' \| 'renewal'`; renewal mode excludes only the family's top payer (CKQ earliest-enrolled tiebreak); registration mode always applies `10% × min(new fee, family's top fee)` immediately, even when the new child is the higher payer (the bridge case) — not yet merged. | `docs/plans/billing-anchor-and-sibling-discount-plan.md` |
