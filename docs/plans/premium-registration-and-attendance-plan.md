...
3. **`Visit` covers group classes and trials only.** Private lessons keep their existing `PrivateClassSession`/`PrivateClassCharge` system, untouched. **Superseded 2026-09-24 by [ADR 010](../decisions/010-universal-visit-ledger.md)** — `Visit` now carries a `serviceId` and records private-lesson attendance too. Decision 2's "no `serviceId`" is superseded by the same ADR.
