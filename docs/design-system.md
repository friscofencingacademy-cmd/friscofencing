...
| `useDebouncedValue` | `lib/hooks/useDebouncedValue.ts` | `useDebouncedValue(value, ms)` — the one debounce (`docs/plans/coach-pack-pricing-plan.md` D14 f). Used by `/admin/subscriptions`' search box and `PackEditor`'s preview; never hand-roll a `setTimeout` debounce in a page. |
...
| `PackEditor` | `app/components/admin/PackEditor/` | The one private-lesson pack editor (`docs/plans/coach-pack-pricing-plan.md` D14 e) — rows of lesson length / sessions / price, a debounced backend preview per row from `POST /coach-contracts/pack-quotes` (rendered verbatim, never computed), `onCanSaveChange` for the parent dialog's Save. Rendered inside the shared `Modal` by both the create-contract and Edit-packs dialogs on `/admin/coach-contracts`. Uses `admin.module.css` plus a token-only module for its row grid. |
