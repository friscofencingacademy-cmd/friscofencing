...
Short by design — this is primarily a data-dense admin/CRM app, plus a small public marketing surface (`/`, `/classes`, `/coaches`, `/calendar`, `/register`, `/login`, see `docs/features/public-site.md`), not a full product design system. Extend this file if a pattern repeats a third time; don't pre-build for patterns that don't exist yet.
...
- **Sidebar** (`--sidebar-w` 220px desktop): brand block (wordmark + gold dot, role label) → nav list → sidebar footer (`Welcome, {firstName}` + logout). `TOP_LEVEL_ITEMS` (Dashboard, Users) render standalone above the collapsible `NAV_SECTIONS` — **Programs** (Classes, Levels, Schedules, Holidays, Subscriptions, Private Classes, Coach Contracts), **Billing** (Prices), **Places** (Locations), **Content** (Spotlights) — both hardcoded inside `layout.tsx`, not data-driven. A section auto-opens when it contains the active route (`useEffect` on `pathname`) and collapses the others; a collapsed section's items stay in the DOM (only a CSS class toggles visibility), so a test can query them without opening the section first.
...
### Calendar (`docs/plans/calendar-view-plan.md`) — adopted 2026-09-25

One shared, controlled component, `CalendarView` (`app/components/calendar/`), used by every calendar page (public `/calendar` now; the parent and admin calendars reuse it). Rules:

- **The backend places every event.** Each event carries its Central `day` (`'YYYY-MM-DD'`); the view groups by that string only, never from `startsAt`, and never with browser-local `Date` getters. All the view's date logic lives in one file, `lib/calendarView.ts` (month grid, request range, URL state, grouping), on calendar-day strings. Times are only formatted, in Central (`formatInstant`). Prices are the backend's, formatted with `formatMoney`.
- **Two views**: a month grid (a plain `<table>` — 7 weekday columns, 6 weeks, up to 3 events per day then "+N more", which opens that day in the list) and a list of days (the default at ≤768px, the portal shell's phone breakpoint). The screen-width default is read after mount so the first render matches the server's.
- **State lives in the URL** (`?month=YYYY-MM&coach=<id>&type=all|group|private&view=month|agenda`), so a filtered calendar is a shareable link. Invalid values fall back to defaults. The page owns the URL; the component only calls `onStateChange`.
- **Toolbar**: previous / Today / next (next stops at the backend's `horizonTo`; previous stops at the current month unless `horizonTo` is `null`, i.e. admin), the month as an `h2`, a coach `<select>` from the backend's `coaches` list, and type and view pill `radiogroup`s.
- **Color is never the only signal**: every event chip writes its kind out ("Class", "Open slot", "Booked", "Your lesson", "Holiday"); a linked chip's accessible name is the full description in the same order as its visible text.
...
| `useDebouncedValue` / `useContractPreview` | `lib/hooks/` | `useDebouncedValue(value, ms)` — the one debounce (`docs/plans/coach-pack-pricing-plan.md` D14 f), used by `/admin/subscriptions`' search box and by `useContractPreview`; never hand-roll a `setTimeout` debounce in a page. `useContractPreview` — the coach-contract editor's one preview (§8 V7): pack-row helpers, the debounced `POST /coach-contracts/preview`, each row's status line, the lesson prices under the rate, and `canSave`. |
...
| `PackEditor` | `app/components/admin/PackEditor/` | The one private-lesson pack editor (`docs/plans/coach-pack-pricing-plan.md` D14 e) — presentational (§8 V7): rows of lesson length / sessions / price, Add / Remove, and the status line `useContractPreview` gives each row. The Packs section of the coach-contract dialog. Uses `admin.module.css` plus a token-only module for its row grid. |
...
Bootstrap, Playfair-style display serif, the discovery page pattern, the four-surface notification hierarchy, **animated stat bars/counters**, testimonial walls, stylelint CI enforcement of these rules, CKQ's membership/premium-upsell machinery, view-as impersonation, and agent chat. The animated-stats and testimonial-wall exclusions are a deliberate call on the now-shipped public site (`docs/features/public-site.md`), not an MVP-scope placeholder — every public page's copy/numbers must trace to a real backend field, which rules out an unverifiable counter or a vague-praise carousel by construction, not just by taste. The rest exist to serve a portal-scale feature set this app doesn't have; revisit only if that changes.

**Reversed 2026-09-25 — calendar pages.** This list used to say "the discovery/calendar page patterns". The owner asked for a calendar (`docs/plans/calendar-view-plan.md`), so calendar pages are now adopted — built in-house, not CKQ's pattern and not a library; see "Calendar" under Page patterns above. A deliberate reversal recorded here, not a silent drift (same precedent as the 2026-08-29 radius change). The discovery page pattern stays not adopted.
