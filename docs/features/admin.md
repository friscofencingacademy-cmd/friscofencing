...
- **Not due** (full month only): "Not due until {date}" disables the period — except in dunning (`retryCount > 0`), since `retryOne` never gates on `nextBillingDate`; the dunning note ("Retry attempt N of 3 — charging the locked amount from the failed charge") shows the LOCKED failed-row amount for a card retry, or lets the admin record a manual payment instead (which clears the dunning cycle, D6).
- **Already paid this month** (prorated only): a ledger-sourced success alert — "This month is already paid — $X on {date} (card/manual)" — reads `previewRenewal`'s `monthAlreadyPaid`, disables Confirm. Not possible for full-month, which self-corrects via `due` the moment any pathway completes a charge for that period.
