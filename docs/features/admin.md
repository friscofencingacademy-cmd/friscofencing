...
- **Columns**: Student | Parent (email sub-line) | Class (name + level sub-line) | Schedule (day/time + coach sub-line) | Next billing | **Last payment** (renamed from "Last charge" — docs/plans/payment-airtight-plan.md D11; `$X` sourced from the Registration ledger's most recent completed row, fee included, plus a muted `Manual` chip when that row's `chargeMethod` is `'manual'`, and a `10% sibling` chip when `lastSiblingDiscountApplied`) | Status chip (`Active` / crimson `Cancels <date>` / muted `Cancelled`) | Actions.
- **Actions**: **Payment History** (every row, both admin and superadmin — see below), **Change Schedule** (active or pending-cancel, **hidden for a premium subscription** — `row.isPremium`, see below), **Cancel** (active only), **Reactivate** (pending-cancel only), **Charge** (superadmin only, any non-cancelled row — see below).

**Payment History** (docs/plans/manual-charge-and-pdf-invoice-plan.md's 2026-08-31 addendum, built alongside the pdfkit-on-Vercel bundling fix) — opens the shared `Modal` around the exact same `PaymentHistoryTable` component the parent portal's own `/parent/billing` page uses, showing the row's **whole family's** history (every child under that row's parent, not just this one student) — same scope, same invoice Download links, same everything a parent sees themselves. Backend: `GET /registrations/history` now accepts an admin/superadmin-only `?parentId=` query param (`registration.controller.js`'s `history`); a parent role always gets their own `req.user._id` regardless of any `parentId` it sends — that parameter is never honored for that role. No new backend logic beyond that access-scoping branch — `listHistory(parentId)` and `GET /:id/invoice` (already admin-accessible) are reused verbatim.
...
**Charge (manual per-subscription billing, superadmin only)** — `docs/plans/manual-charge-and-pdf-invoice-plan.md` PR 1, added 2026-08-30 while scheduled renewal runs are paused by owner decision; grown into a card/manual × full/prorated dialog by `docs/plans/payment-airtight-plan.md` D4. `renewal.service.js`'s `previewRenewal`/`chargeNow`/`chargeProratedNow`/`recordManualPayment`, routed at `GET/POST /api/v1/subscriptions/:id/charge-preview`, `/:id/charge`, and `/:id/record-payment`, all superadmin-only (same sensitivity class as `/admin/settings`). Zero new charge logic beyond `chargeProratedNow` itself — every existing guard (fresh re-fetch, per-month ledger dedup via Guard B's `periodMonth` key, stale-pending recovery, idempotency keys, dunning state, emails) applies unchanged to every path. Clicking Charge opens a dialog that loads the read-only preview (never calls Stripe) and, once loaded, offers two independent choices:

- **How to collect** — *Charge card on file* (the existing Stripe path) or *Record offline payment* (an amount input, prefilled from the selected period's computed total but editable, plus a required note — e.g. "Paid by check #1042" — no Stripe call; footer button reads "Record Payment").
- **Period covered** — *Full month* (`currentPeriodEnd → +1 month`, the standard renewal — still gated on `due`, same as before) or *Prorated from today* (`computeProration()` anchored at today rather than `currentPeriodEnd` — an off-cycle catch-up tool for a lapsed/mid-month restart, deliberately never gated on `due`; disabled when the schedule/level chain can't be resolved).

Both amounts/breakdowns come via the SAME `resolveMonthlyFee`/`calculateChargeAmount`/`computeProration` functions the real charge itself calls, so the preview can never disagree with reality (the standing rule, ADR 001's 2026-08-23 addendum). Additional dialog states:
- **Card on file** (`Visa •••• 4242`) or a red "No card on file — this charge will fail." warning that disables Confirm — only shown/enforced for the card method.
- **Not due** (full month only) / **Already paid this month** (prorated only, ledger-sourced via `previewRenewal`'s `monthAlreadyPaid`) — both disable Confirm, and both now render as a single, prominent success `Alert` at the TOP of the dialog body, above the method radios, rather than a barely-visible muted line near the bottom (docs/plans/booking-and-private-class-fixes-plan.md §5, 2026-08-31 — an owner report that Confirm "shows all the options but is always greyed out"; Guard B itself — one payment per subscription per calendar month, built after a real $403.82 double-charge incident — is correct and unchanged, no override was added, only the explanation was promoted). "Not due" reads "Already paid through this period. Not due until {date} — the Confirm button unlocks then."; "already paid this month" reads "This month is already paid — $X on {date} (card/manual)." Neither renders during a dunning card retry or a pending-cancel finalize, which have their own Alerts. Full-month "not due" is bypassed in dunning (`retryCount > 0`), since `retryOne` never gates on `nextBillingDate`; the dunning note ("Retry attempt N of 3 — charging the locked amount from the failed charge") shows the LOCKED failed-row amount for a card retry, or lets the admin record a manual payment instead (which clears the dunning cycle, D6). A small inline hint under the Note field ("A note is required to record an offline payment.") shows whenever the manual note is empty, so a legitimately due period recording a manual payment never *looks* stuck for no visible reason.
- **Due + pending-cancel**: unchanged — "Processing will finalize the cancellation — nothing is charged," Confirm relabeled "Finalize"; no method/period choice shown.
- Confirm/Record Payment renders the real outcome (`charged`, `failed_payment` + next retry date, `cancelled_finalized`, `invalid_amount`/`invalid_note` for a malformed manual entry, etc. — the dialog never invents its own vocabulary) and refreshes the list.
...
Every contract **version** is listed (`docs/plans/coach-pack-pricing-plan.md` §8), each coach's versions together, newest first. Columns: coach, `$/hr` billed to parent, `$/hr` coach compensation, default session duration, **Packs** (one line per pack, "10 × 30 min — $300.00"), status (**Current** / **Replaced** / **Ended**), Since, Until. The current version has **Edit** and **Deactivate**; older versions have no actions — they are history. **Add Contract** is only for a coach with no current contract: its coach list leaves out coaches who have one (the backend refuses a second with 409 "edit it instead").

**One dialog, two sections** (Add and Edit share it). **Rates**: rate billed to parents ($/hr), coach compensation ($/hr), default lesson length — with the backend's lesson prices shown under the rate as you type ("Lesson prices: 30 min $30.00 · 60 min $60.00", for the default length, every pack length and every length the coach publishes). **Packs** (`PackEditor`): each row has lesson length, sessions and price, and under it the backend's own check — "$30.00 per lesson · saves $25.00 (8%)", or the exact message saving would return ("10 × 30 min: price must be between $162.50 and $324.99"). Both come from one debounced call, `POST /coach-contracts/preview`, owned by the `useContractPreview` hook; the form computes no prices. Save is disabled while a row is incomplete, being checked or refused, and (on Edit) while nothing has changed; a **failed** preview shows "Preview unavailable" and does not block Save — the backend's 400 on save is the rule and is shown verbatim. **Saving an Edit** keeps the current version as a read-only "Replaced" line and starts a new current version from today (`POST /coach-contracts/:id/revisions`); past purchases keep what they paid, and a parent holding a quote from the old version is asked to review the new prices instead of being charged them. **Deactivate** ends the current version with no successor ("Ended"). A pack is a fixed total price for a number of lessons of one length, offered only on that coach's slots of that length. Packs and rates are admin + superadmin, like every contract route (plan D13).
...
## Calendar (`GET /calendar`, backend only so far)

`docs/plans/calendar-view-plan.md` — PR 1 (backend) built 2026-09-25; the `/admin/calendar` page lands in PR 3. `GET /api/v1/calendar?from&to&coachId&type` (admin/superadmin): the public calendar's events plus every confirmed private booking (`kind: 'private-booked'`, with `students`), group sessions already started or on a holiday (a holiday-date session is `isHoliday: true` with `holidayName`), and one all-day `kind: 'holiday'` event per holiday day — holidays shown whatever the coach/type filter. No horizon: `horizonTo` is `null` and past months are allowed (still at most 42 days per request).
...
**Not-yet-open rows** (docs/plans/duplication-cleanup-plan.md PR A) — attendance opens on the session's own calendar day (Central), for admins and coaches alike, with no late-marking cutoff. A session whose day hasn't started is annotated `attendanceOpen: false` (additive, alongside `isHoliday`): its row keeps the student count but shows a muted `Not open yet` chip instead of the "Mark Attendance" link, and the attendance page itself renders a blocked alert ("Attendance opens on {date}") with no roster or Save button. Blocked only on an explicit `false` (absent = open, same permissive default as `isHoliday`); a holiday takes precedence when both apply. Both attendance-writing endpoints (`PATCH .../attendance`, walk-in `POST .../students`) 400 on such a session — the rule and why it differs from private-class attendance are in `docs/decisions/009-utc-date-storage-standard.md`'s attendance addendum.
...
**Future reuse note** (docs/plans/payment-airtight-plan.md D10): there is no per-user detail page today — Edit is a dialog, not a route. When one is built, `app/components/ui/PaymentHistoryTable/` (built for the parent-facing `/parent/billing` page, `docs/features/parent-portal.md`) can mount there verbatim — it's props-driven (`rows`, `loading`), does no fetching of its own, and its row shape doesn't care whether the rows came from a parent's own `GET /registrations/history` or a future admin-scoped equivalent (e.g. `GET /registrations/history?parentId=`). No component changes anticipated, only a new backend endpoint + fetch call in that future page.
