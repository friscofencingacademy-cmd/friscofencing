...
| `calendar.spec.ts` | The public `/calendar`: a class and an open slot on the right days, filtering by coach (kept in the URL), a logged-out click landing on login with the wizard link in `next`, a parent's click opening the wizard with the date already picked, and zero-tolerance axe scans of both the month grid and the list | Clock pinned to `CALENDAR_NOW` (`fixtures/mock-api.ts`), so the default month is always October 2026. `docs/plans/calendar-view-plan.md` |
