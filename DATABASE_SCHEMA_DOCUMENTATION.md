...
Index: `{ coachId: 1, isActive: 1 }`. Creating a new contract for a coach deactivates their previous active one (service layer, not a schema constraint) — one active contract per coach; the previous contract's packs carry over (as new pack ids) unless the request sends its own list, and are re-checked against the new rate. The rate fields are never edited once created — a rate change is a new contract, and the old one is only deactivated, an immutable rate-audit record. **The one exception is `privateLessonPacks`**, replaceable on the active contract (`PUT /coach-contracts/:id/packs`, plan D7): an unchanged pack keeps its `_id`, an edited one gets a new `_id`, so a quote for the old price can never be charged at the new one. Past purchases are unaffected — each one's price is pinned on its `Registration` row.
